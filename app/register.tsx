...
export default function RegisterScreen() {
  ...
  async function handleRegister() {
    ...
    try {
      ...
      // registerUser já faz login automático e salva a sessão
      await registerUser(username.trim(), password);

      // Vai direto para home — sem precisar logar de novo
      router.replace('/home');
    } catch (err: any) {
      // Exibe a mensagem de erro vinda da API (ex: usuário já existe)
...
const styles = StyleSheet.create({
  ...
  logoBox: {
    width: 40, height: 40, borderRadius: 12,
    backgroundColor: '#e8ff47', alignItems: 'center', justifyContent: 'center',
  },
  logoText: { fontSize: 18, color: '#0a0a0a' },
  appName: { fontSize: 12, fontWeight: '700', letterSpacing: 5, color: '#fff' },
  card: {
    backgroundColor: '#141414', borderRadius: 20,
    padding: 28, borderWidth: 1, borderColor: '#222',
  },
  title: { fontSize: 24, fontWeight: '700', color: '#fff', letterSpacing: -0.5, marginBottom: 6 },
  ...
  strengthLabel: { fontSize: 11, color: '#888', marginLeft: 4, width: 36 },
  btn: {
    backgroundColor: '#e8ff47', borderRadius: 12,
    paddingVertical: 15, alignItems: 'center', marginTop: 8, marginBottom: 20,
  },
